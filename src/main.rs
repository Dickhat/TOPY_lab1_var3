...
impl HookeJeevesApp {
    ...
    fn f2(x: &[f64]) -> f64 {
        4.0 * x[0].powi(2) + 3.0 * x[1].powi(2) + x[2].powi(2) + 4.0 * x[0] * x[1]
            - 2.0 * x[1] * x[2]
            - 16.0 * x[0]
            - 4.0 * x[2]
    }
    ...
    fn optimize(&mut self) {
        ...
        while delta > self.epsilon {
            ...
            for j in 0..n {
                ...
                steps.push(ExploratoryStep {
                    ...
                    yj: if j == 0 {
                        ...
                        steps.last().unwrap().yj.clone()
                    },
                    fyj: if j == 0 {
                        fxk
                    } else {
                        steps.last().map(|s| s.fyj).unwrap_or(fxk)
                    },
                    dj,
            ...
            if iter_count > 1000 {
                self.status_message =
                    "Предупреждение: достигнуто максимальное число итераций".to_string();
                break;
            }
        }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            if self.computation_done {
                ...
                // 1. ГРАФИК — всегда показывается для F1
                if self.function_choice == 0 {
...
fn main() -> eframe::Result<()> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default().with_inner_size([1600.0, 1050.0]),
        ..Default::default()
    };
